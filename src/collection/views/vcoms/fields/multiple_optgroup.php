...
?>
<div>
    <select name="<?php echo $this->field->name ?>[]" multiple id="<?php echo $this->field->id ?>" class=" <?php echo $this->field->formClass?>" <?php echo $this->field->required?> <?php echo $this->field->autocomplete;?>>
        <?php foreach( $this->field->options as $opt )
        {
            ...
            if (is_array($opt['option']) && ($opt['option']))
            {
                ...
                foreach ($opt['option'] as $option)
                {
                    ...
                    echo is_array($this->field->value) && in_array($value, $this->field->value) ? 'selected="selected" >' : '>';
                    echo $option->text;
                    echo '</option>';
                }
                echo '</optgroup>';
